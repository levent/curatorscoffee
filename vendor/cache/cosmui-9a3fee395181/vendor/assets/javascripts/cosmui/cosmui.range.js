...
if (window.cosmUI) {  
  ...
  // Definition
  //
  // This cosmUI plugin is a middleware for frequency decoder's Accessible Unobtrusive “Slider/HTML5 range polyfill"
  //
   
  cosmUI.range = (function( $ ){
    "use strict"
    
    // ----------------------------------- 
    // Private
    
    var globalSelector = '[data-cosmui*="range"]',
        interval,
        ...
        change = function ( obj ) {
          var $this = $(obj.elem),
              $output = $('#'+ $this.attr('id').replace(/range-/,'range-output-'));
          $this.trigger('cosmui.range.change', obj.value);
          $output.html( $this.find(':selected').text() );
        },
        ...
        created = function ( obj ) {
          var $this = $(obj.elem),
              $range = $this.siblings('.fd-slider');
          $range.css('display','inline-block');
          setTimeout(function(){ $this.trigger('cosmui.range.created',$this) },0);
          
          // create the output
          $range.before( $('<span />', { 'id' : $this.attr('id').replace(/range-/,'range-output-'), 'class' : 'graph-duration-output', html : $this.find(':selected').text() }) );
        },
        ...
        init = function ( e ) {
          ...
          if (!$('html').hasClass('oldie') && cosmUI.screenSize() > 880 && !Modernizr.touch) {
            // use plugin only if scenario is ideal
            $(globalSelector).each(function(){
              $this = $(this);
              
              $this.attr('id','cosmUI-range-'+ counter);
              
              fdSlider.createSlider({
                inp:document.getElementById('cosmUI-range-'+ counter),
                animation:"tween",
                hideInput:true,
                callbacks: {
                  "dragend"  : [change],
            ...
            $(globalSelector).each(function(){
              $this = $(this);
              $this.trigger('cosmui.range.created',$this);
              $this.on('change',function(){
                var value = $(this).val();
                $(this).trigger('cosmui.range.change',value);
              });  
